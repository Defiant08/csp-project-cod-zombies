namespace SpriteKind {
    export const portaldoor = SpriteKind.create()
}
...
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile`, function (sprite, location) {
    ...
    current_map += 1
    new_player_1 = sprites.create(img`
        . . . . . . f f f f . . . . . . 
        . . . . f f f 2 2 f f f . . . . 
        . . . f f f 2 2 2 2 f f f . . . 
        ...
        `, SpriteKind.Player)
    new_player_1.ay = 300
    controller.moveSprite(new_player_1, 100, 100)
    jump = 0
    Map_Changer()
    scene.cameraFollowSprite(new_player_1)
})
...
function Map_Changer () {
    if (current_map == 0) {
        tiles.setCurrentTilemap(tilemap`level1`)
        tiles.placeOnRandomTile(Player_1, sprites.dungeon.floorDarkDiamond)
        for (let pink_block of tiles.getTilesByType(assets.tile`myTile1`)) {
            tiles.setTileAt(pink_block, assets.tile`transparency16`)
        }
        spawn_point = tiles.getTilesByType(sprites.dungeon.floorLight1)
        for (let index = 0; index < 8; index++) {
            en = sprites.create(enemies._pickRandom(), SpriteKind.Enemy)
            tiles.placeOnTile(en, spawn_point.removeAt(randint(0, spawn_point.length - 0)))
            en.follow(Player_1, 70)
        }
    } else if (current_map == 1) {
        tiles.setCurrentTilemap(tilemap`level9`)
        tiles.placeOnRandomTile(new_player_1, assets.tile`myTile1`)
...
let en: Sprite = null
let spawn_point: tiles.Location[] = []
let new_player_1: Sprite = null
let jump = 0
let enemies: Image[] = []
